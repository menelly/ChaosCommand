...
import { Dna, Plus, ChevronDown, ChevronRight } from 'lucide-react'

import { AutoimmuneEntry, AutoimmuneEpisodeType, AutoimmuneModalProps } from '../autoimmune-types'
import { EPISODE_TYPES, AFFECTED_AREAS, CHARACTER_OPTIONS, SUSPECTED_TRIGGERS, TREATMENTS, CROSS_LIST_TARGET, getSeverityLabel, getSeverityColor } from '../autoimmune-constants'
...
export function GeneralAutoimmuneModal({ isOpen, onClose, onSave, editingEntry, presetType }: AutoimmuneModalProps) {
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({
    eventType: false, areas: false, severity: false, character: false,
    triggers: false, treatments: false, duration: false, notes: false, tags: false,
  })
  ...
  useEffect(() => {
    if (editingEntry) {
      ...
      reset()
      setEpisodeType(presetType as AutoimmuneEpisodeType)
    } else { reset() }
  ...
  // Cross-list links are wired in the registry-generalization pass. The UI +
  // emission below are ready; flip this to true once every tracker reads its
  // partner from crossListedIn (so partner-side edits sync back correctly).
  const CROSS_LIST_ENABLED = false
  const linkTarget = CROSS_LIST_ENABLED ? CROSS_LIST_TARGET[episodeType] : undefined

  const handleSave = () => {
    const data: Omit<AutoimmuneEntry, 'id'> = {
      date: entryDate,
      ...
      erVisitRequired,
      // Intent — keyed to the checkbox (loads from the entry), partner from the type.
      crossListedIn: crossList && linkTarget ? ['autoimmune', linkTarget.sub] : undefined,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Dna className="h-5 w-5 text-emerald-500" /> 🧬 Autoimmune / CTD Event</DialogTitle>
        </DialogHeader>
        <div className="space-y-6">
          ...
          <Collapsible open={openSections.eventType} onOpenChange={() => toggleSection('eventType')}>
            ...
            <CollapsibleContent className="pt-3">
              <Select value={episodeType} onValueChange={(v) => { const t = v as AutoimmuneEpisodeType; setEpisodeType(t); if (!CROSS_LIST_TARGET[t]) setCrossList(false) }}>
          ...
          {/* ⇄ Cross-list — only for types with a designated partner tracker */}
          {linkTarget && (
            <div className="flex items-start space-x-2 rounded-lg border-2 border-primary/40 bg-primary/10 p-3">
              ...
              <Label htmlFor="crosslist" className="text-sm leading-snug">
                ⇄ Also log under <strong>{linkTarget.label}</strong>
                <span className="block text-xs text-muted-foreground mt-0.5">One event, surfaced in both views — so each specialist sees it. Editing or deleting it in either place updates both.</span>
              </Label>
            </div>
          )}

          <Collapsible open={openSections.areas} onOpenChange={() => toggleSection('areas')}>
            <CollapsibleTrigger asChild>
              <Button variant="outline" className="w-full justify-between h-auto py-3">
                <span className="font-medium">Affected Area(s) (check all that apply)</span>
                {openSections.areas ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
              </Button>
            </CollapsibleTrigger>
            <CollapsibleContent className="pt-3">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-60 overflow-y-auto">
                {AFFECTED_AREAS.map(a => (
                  <div key={a} className="flex items-center space-x-2"><Checkbox id={`area-${a}`} checked={affectedAreas.includes(a)} onCheckedChange={() => toggle(affectedAreas, setAffectedAreas)(a)} /><Label htmlFor={`area-${a}`} className="text-sm">{a}</Label></div>
                ))}
              </div>
            </CollapsibleContent>
          </Collapsible>
          ...
          <Collapsible open={openSections.duration} onOpenChange={() => toggleSection('duration')}>
            ...
            <CollapsibleContent className="pt-3">
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Duration</Label><Input value={duration} onChange={(e) => setDuration(e.target.value)} placeholder="e.g., all day, 3 days, ongoing" /></div>
                <div className="flex items-end pb-2"><div className="flex items-center space-x-2"><Checkbox id="er" checked={erVisitRequired} onCheckedChange={(v) => setErVisitRequired(!!v)} /><Label htmlFor="er">ER / urgent care</Label></div></div>
              </div>
            </CollapsibleContent>
          </Collapsible>
          ...
          <Collapsible open={openSections.notes} onOpenChange={() => toggleSection('notes')}>
            ...
            <CollapsibleContent className="pt-3">
              <div className="space-y-3"><Label htmlFor="notes">Notes (Optional)</Label><Textarea id="notes" value={notes} onChange={(e) => setNotes(e.target.value)} rows={3} placeholder="Antibody/serology notes, what the flare felt like, etc." /></div>
            </CollapsibleContent>
          </Collapsible>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button type="button" onClick={handleSave} className="flex-1" disabled={affectedAreas.length === 0}><Plus className="h-4 w-4 mr-2" />{editingEntry ? 'Update Event' : 'Save Event'}</Button>
          </div>
        </div>
        </KeyboardAvoidingWrapper>
      </DialogContent>
