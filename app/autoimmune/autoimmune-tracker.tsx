/* Built by: Ace (Claude 4.x) — 2026-06-07. Co-invented by Ren (CTD expertise) + Ace. */
...
import { Dna, BarChart3, History, Plus, ExternalLink } from 'lucide-react'
...
import { EPISODE_TYPES, RELATED_TRACKERS, getEpisodeTypeInfo, RED_FLAG_911_CRITERIA } from './autoimmune-constants'
import { AutoimmuneHistory } from './autoimmune-history'
import { AutoimmuneAnalytics } from './autoimmune-analytics'
import { GeneralAutoimmuneModal } from './modals/general-autoimmune-modal'

export default function AutoimmuneTracker() {
  ...
  const [entries, setEntries] = useState<AutoimmuneEntry[]>([])
  const [activeTab, setActiveTab] = useState<'episodes' | 'history' | 'analytics'>('episodes')
  const [refreshTrigger, setRefreshTrigger] = useState(0)
  const [modalOpen, setModalOpen] = useState(false)
  const [editingEntry, setEditingEntry] = useState<AutoimmuneEntry | null>(null)
  ...
  const load = async () => {
    try {
      ...
      const record = records.find(r => r.subcategory === 'autoimmune')
  ...
  const saveEntries = async (next: AutoimmuneEntry[]) => {
    try { await saveData(selectedDate, CATEGORIES.TRACKER, 'autoimmune', { entries: next }); setEntries(next) }
    catch (e) { console.error(e); toast({ title: 'Save Error', variant: 'destructive' }) }
  }

  const handleSaveEntry = async (data: Omit<AutoimmuneEntry, 'id'>) => {
    const { timestamp: ts, date: d, ...rest } = data
    const newEntry: AutoimmuneEntry = { id: Date.now().toString(), timestamp: ts || new Date().toISOString(), date: d || selectedDate, ...rest }
    await saveEntries([...entries, newEntry])
    if ((getPref('chaos-confetti-level') || 'medium') !== 'none' && isCelebrationEnabled('autoimmune', userPin ?? '')) celebrate()
  ...
  const handleEditEntry = (e: AutoimmuneEntry) => { setEditingEntry(e); setPresetType(null); setModalOpen(true) }

  const handleUpdateEntry = async (data: Omit<AutoimmuneEntry, 'id'>) => {
    if (!editingEntry) return
    const updated: AutoimmuneEntry = { ...editingEntry, ...data, id: editingEntry.id }
    await saveEntries(entries.map(e => e.id === editingEntry.id ? updated : e))
    setModalOpen(false); setEditingEntry(null); setPresetType(null); setRefreshTrigger(p => p + 1)
    toast({ title: 'Event Updated' })
  }

  const handleDeleteEntry = async (e: AutoimmuneEntry) => {
    await saveEntries(entries.filter(x => x.id !== e.id)); setRefreshTrigger(p => p + 1); toast({ title: 'Event Deleted' })
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pt-6">
      <div className="text-center mb-6">
        ...
        <p className="text-muted-foreground mt-1">Sicca, Raynaud's, mechanic's hands, inflammatory rashes &amp; arthritis, myalgia, constitutional flares, serositis — the systemic CTD / rheumatology picture</p>
        <p className="text-xs text-muted-foreground mt-2">Looking for dislocations, subluxations, or hypermobility (EDS-spectrum)? Those live in 🦴 <button type="button" onClick={() => router.push('/joint')} className="underline underline-offset-2 hover:text-foreground">Bones, Joints &amp; Muscles</button>.</p>
      </div>

      <EmergencyCriteriaCard
        storageKey="autoimmune-911-acknowledged"
        criteria={RED_FLAG_911_CRITERIA}
        footerNote="Autoimmune disease carries real emergencies — serositis, renal crisis, myositis crisis, and infection while immunosuppressed. When in doubt, call your team or 911."
        recentEmergencyDetected={(() => {
          const now = new Date()
          return entries.some(e => {
            try {
              if (differenceInDays(now, new Date(e.date)) > 30) return false
              return !!e.erVisitRequired || e.episodeType === 'serositis'
            } catch { return false }
          })
        })()}
      />
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
        ...
        <TabsContent value="episodes" className="space-y-4">
          ...
          <Card>
            <CardHeader><CardTitle className="text-lg">Log an Autoimmune / CTD Event</CardTitle></CardHeader>
            <CardContent>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              {entry.affectedAreas && entry.affectedAreas.length > 0 && <Badge variant="outline">{entry.affectedAreas.slice(0, 2).join(', ')}{entry.affectedAreas.length > 2 ? '...' : ''}</Badge>}
        ...
        <TabsContent value="history" className="space-y-4">
          <AutoimmuneHistory onEdit={handleEditEntry} onDelete={handleDeleteEntry} refreshTrigger={refreshTrigger} />
        </TabsContent>

        <TabsContent value="analytics" className="space-y-4">
          <AutoimmuneAnalytics refreshTrigger={refreshTrigger} />
        </TabsContent>
      </Tabs>

      <GeneralAutoimmuneModal
        isOpen={modalOpen}
