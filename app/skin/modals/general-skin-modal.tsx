            ...
            return (
              <div className="border-2 border-destructive bg-destructive text-destructive-foreground rounded-lg p-4 space-y-3">
                ...
                <div className="ml-7 pt-2 border-t border-destructive-foreground/30 space-y-2">
                  <p className="text-sm text-destructive-foreground"><strong>If happening RIGHT NOW:</strong> call 911 if breathing/throat affected. Documenting can wait.</p>
                  <p className="text-sm text-destructive-foreground"><strong>If this is in the PAST and resolved:</strong> document carefully and follow up with your medical team / dermatology.</p>
                </div>
                {measures.length > 0 && <div className="ml-7 pt-2 border-t border-destructive-foreground/30"><p className="text-sm font-semibold text-destructive-foreground mb-1">💪 While waiting for EMS:</p><ul className="space-y-2 text-sm text-destructive-foreground">{measures.map((m, i) => <li key={i}>• {m}</li>)}</ul></div>}
              </div>
            )
          })()}
