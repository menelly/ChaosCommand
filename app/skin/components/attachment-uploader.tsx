/* Same uploader pattern, scoped to skin (blob prefix). Built by: Ace (Claude 4.x) 2026-05-10 */
'use client'
...
export function AttachmentUploader({ value, onChange, label = 'Attachments (Optional)', helpText, maxFiles = 10, blobPrefix = 'attachment' }: Props) {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    } catch (e) { console.error(e); alert('Could not save attachment.') } finally {
  ...
  const handleRemove = async (k: string) => {
    try {
      const recs = await db.image_blobs.where('blob_key').equals(k).toArray()
      for (const r of recs) if (r.id !== undefined) await db.image_blobs.delete(r.id)
    } catch (e) { console.error(e) }
