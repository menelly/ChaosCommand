/* Built by: Ace (Claude 4.x) — 2026-05-10. Co-invented by Ren (vision) and Ace (implementation). */
...
export default function SkinTracker() {
  ...
  const load = async () => {
    try {
      ...
      const record = records.find(r => r.subcategory === 'skin')
  ...
  const saveEntries = async (next: SkinEntry[]) => {
    try { await saveData(selectedDate, CATEGORIES.TRACKER, 'skin', { entries: next }); setEntries(next) }
  ...
  const handleSaveEntry = async (data: Omit<SkinEntry, 'id'>) => {
    ...
    if ((getPref('chaos-confetti-level') || 'medium') !== 'none' && isCelebrationEnabled('skin', userPin ?? '')) celebrate()
  ...
  const handleDeleteEntry = async (e: SkinEntry) => {
    ...
    <div className="max-w-4xl mx-auto space-y-6 pt-6">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)}>
        ...
        <TabsContent value="episodes" className="space-y-4">
          ...
          <Card>
            <CardHeader><CardTitle className="text-lg">Log a Skin Event</CardTitle></CardHeader>
            <CardContent>
        ...
        <TabsContent value="history" className="space-y-4">
          <SkinHistory onEdit={handleEditEntry} onDelete={handleDeleteEntry} refreshTrigger={refreshTrigger} />
        </TabsContent>

        <TabsContent value="analytics" className="space-y-4">
          <SkinAnalytics refreshTrigger={refreshTrigger} />
        </TabsContent>
      </Tabs>
