...
export function AttachmentUploader({
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      for (const key of value) {
        ...
        } catch (e) { console.error('Hydrate fail', e) }
      }
    ...
    return () => {
      cancelled = true
      for (const p of previews) { try { URL.revokeObjectURL(p.url) } catch {} }
    }
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    if (value.length + files.length > maxFiles) { alert(`Maximum ${maxFiles} attachments per event.`); return }
    setUploading(true)
  ...
  const handleRemove = async (blobKey: string) => {
    try {
      ...
      for (const r of records) { if (r.id !== undefined) await db.image_blobs.delete(r.id) }
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {previews.map((p) => (
            <div key={p.blobKey} className="relative border rounded-lg overflow-hidden bg-muted/30">
              ...
              <Button type="button" variant="destructive" size="icon" className="absolute top-1 right-1 h-6 w-6" onClick={(e) => { e.preventDefault(); handleRemove(p.blobKey) }}>
                <X className="h-3 w-3" />
              </Button>
            </div>
          ))}
