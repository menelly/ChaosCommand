...
// Category-identity colors. Plain Tailwind pastels — the per-theme CSS override
// layer (styles/chaos-themes.css) neutralizes these on dark themes so they adapt
// instead of clashing. Light themes keep the pastels; the emoji carries identity
// when dark themes flatten them to a uniform tint. Consistent with every other
// tracker's category colors (all handled by the same override system).
export const CATEGORY_INFO: Record<string, { label: string; emoji: string; color: string }> = {
  ...
  'errands': { label: 'Errands', emoji: '🛒', color: 'bg-rose-100 text-rose-800 border-rose-200' },
