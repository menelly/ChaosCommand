...
export function GUAnalytics({ refreshTrigger }: GUAnalyticsProps) {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      console.error('GU analytics load failed:', e)
    } finally {
      setLoading(false)
    }
  }
  ...
  const stats = useMemo(() => {
    ...
    const retentionCount = allEntries.filter(e => e.retentionSuspected).length
    const infectionCount = allEntries.filter(e => e.infectionSuspected || e.episodeType === 'infection').length
    const hematuriaCount = allEntries.filter(e => e.bloodVisible).length
    const flankFeverCount = allEntries.filter(e => e.flankPain && e.feverPresent).length
    ...
    return {
      total, typeCounts, retentionCount, infectionCount, hematuriaCount,
      flankFeverCount, erCount, severityBuckets, avgSeverity,
    ...
    return (
      <Card>
        <CardContent className="pt-6 text-center text-muted-foreground">
          ...
          <p className="font-medium">No GU events in the selected window.</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            <div className="text-xs uppercase tracking-wide text-muted-foreground">Infections</div>
      ...
      {/* Safety-critical flags */}
      {(stats.retentionCount > 0 || stats.hematuriaCount > 0 || stats.flankFeverCount > 0) && (
        <Card>
          <CardHeader className="pb-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {stats.retentionCount > 0 && (
                <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                  ...
                  <div className="text-sm text-destructive/80 font-medium">Retention events</div>
                  <div className="text-xs text-muted-foreground mt-1">Flag for urology if recurring</div>
                </div>
              )}
              {stats.hematuriaCount > 0 && (
                <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                  <div className="text-2xl font-bold text-destructive">{stats.hematuriaCount}</div>
                  <div className="text-sm text-destructive/80 font-medium">Hematuria events</div>
                  <div className="text-xs text-muted-foreground mt-1">Visible blood — warrants evaluation</div>
                </div>
              )}
              {stats.flankFeverCount > 0 && (
                <div className="p-3 rounded-lg bg-destructive/10 border border-destructive/20">
                  <div className="text-2xl font-bold text-destructive">{stats.flankFeverCount}</div>
                  <div className="text-sm text-destructive/80 font-medium">Flank pain + fever</div>
                  <div className="text-xs text-muted-foreground mt-1">Possible pyelonephritis</div>
                </div>
              )}
            </div>
          </CardContent>
